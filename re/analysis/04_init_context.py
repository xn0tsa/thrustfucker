...
for ins in disasm_all(0x08000000,0x08009938):
    ...
    if ins.mnemonic in ("bl","blx") and ins.operands and ins.operands[0].type==ARM_OP_IMM and ins.operands[0].imm==TGT:
        print("="*64, f"\ncall 0x{TGT:08X} @0x{ins.address:08X}")
        for w in win: print(f"  0x{w.address:08X}  {w.mnemonic:<9} {w.op_str}")
        shown+=1
        if shown>=6: break
